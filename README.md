...
The funtion was created using the key word *sqrt()*. To start this function, an intial guess (which is reasonably close to the true root) is assigned to the variable *x*. The intial guess is equal to the value entered by the user for the first iteration. [Newtons Method](https://www.geeksforgeeks.org/find-root-of-a-number-using-newtons-method/) is an iterative process, therefore, a *while* loop is necessary in the function. In order to count the iterations in the *while* loop, the *count* is initially set to zero. The *while* loop checks for one codition of convergance while the *if* statement checks for one other condition of convergance.

In the line *if abs(root - x) < 1e-9:* the [abs](https://www.w3schools.com/python/ref_func_abs.asp) function returns the absolute value of the difference between the current approximation of the sqaure root (*root*) and the previous approximation of the sqaure root (*x*). Absolute value refers to the distance of a number from zero, regardless of the direction. The value *1e-9* acts as a tolerance or threshold for when the appproximation is considered accurate enough. Therefore, the *if* statement checks whether the difference between *root* and *x* is less than the threshold *1e-9*. The loop is broken if the difference is less than the threshold. 
